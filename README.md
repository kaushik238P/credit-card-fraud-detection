...
```mermaid
graph TD
    User([User / Client]) -->|Interact| Streamlit[Streamlit UI]
    User -->|Direct HTTP POST| FastAPI[FastAPI Backend]
    Streamlit -->|HTTP POST JSON| FastAPI
    
    subgraph FastAPI Container
        FastAPI -->|1. Validate Schema| Validation[Validation Layer]
        Validation -->|2. Derive Features| FE[Feature Engineering Layer]
        FE -->|3. Encode & Scale| Preprocessing[Preprocessing Layer]
        Preprocessing -->|4. Predict Proba| ModelEngine[CatBoost Model Engine]
        ModelEngine -->|5. Apply Decision Threshold| Decision[Threshold Evaluator]
...
| **FastAPI**        | Backend API      | High-performance ASGI framework for serving REST APIs                             |
| **Streamlit**      | Frontend         | Interactive web dashboard for prediction and system monitoring                    |
| **CatBoost**       | Model Engine     | Production gradient-boosted decision tree model for fraud prediction              |
...
The project includes a multi-page **Streamlit** dashboard for interacting with the fraud detection system.
...
- **Structured Logging**: Centralized application logging with configurable text or JSON output, optimized for containerized deployments.
- **Health Monitoring**: Provides `/health/liveness` and `/health/readiness` endpoints to monitor service availability and model readiness.
- **MLflow Integration**: Tracks experiments, parameters, metrics, and model artifacts for reproducible machine learning workflows.
...
| Attribute | Value |
|-----------|-------|
| **Dataset** | Credit Card Fraud Transaction Dataset |
| **Source** | Kaggle |
| **Total Records** | 1,296,675 |
| **Input Features** | 23 |
| **Target Variable** | `is_fraud` (0 = Legitimate, 1 = Fraud) |
| **Fraud Samples** | 7,506 (0.5789%) |
| **Legitimate Samples** | 1,289,169 (99.4211%) |
| **Class Distribution** | Highly Imbalanced (~172:1 Legitimate to Fraud) |

The severe class imbalance motivated the use of class weighting and threshold optimization during model training.
---
